			   ...
			   unsigned group)
{
	...
	if (function == FUNC_ADC && (pin->analog & LPC18XX_ANALOG_PIN)) {
		...
		/* Disable bias and input in pinconf register */
		writel(0x10, scu->base + pin->offset);
...
static void lpc18xx_pctl_pin_dbg_show(struct pinctrl_dev *pctldev,
					  struct seq_file *s,
					  unsigned offset)
{
	seq_printf(s, "XXX %s", dev_name(pctldev->dev));
}
...
static const struct pinctrl_ops lpc18xx_pctl_ops = {
	...
	.pin_dbg_show		= lpc18xx_pctl_pin_dbg_show,

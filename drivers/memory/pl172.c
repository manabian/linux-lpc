...
static int pl172_timing_prop(const struct device_node *np, const char *name,
			     struct amba_device *adev, u32 reg_offset, u32 max,
			     int start)
{
	struct pl172 *pl172 = amba_get_drvdata(adev);
	int cycles;
	u32 val;
...
#if defined(DEBUG)
	val = readl(pl172->base + reg_offset);
	dev_info(&adev->dev, "%s: %u cycle(s)\n", name, val + start);
#endif
	return 0;
}

static int pl172_configure_static(u32 cs, struct amba_device *adev,
				  struct device_node *np)
{
	...
	if (!of_property_read_u32(np, "mpmc,memory-width", &cfg)) {
		if (cfg == 8) {
			...
			dev_err(&adev->dev, "Invalid memory width on %u\n", cs);
			return -EINVAL;
		}
	} else {
	...
	dev_dbg(&adev->dev, "Clock rate is %lu kHz\n", pl172->rate);

	/* MPMC static memory timing */
	ret = pl172_timing_prop(np, "mpmc,write-enable-delay", adev,
				MPMC_STATIC_WAIT_WEN(cs),
				MPMC_STATIC_WAIT_WEN_MAX, 1);
	if (ret)
		goto fail;

	ret = pl172_timing_prop(np, "mpmc,output-enable-delay", adev,
				MPMC_STATIC_WAIT_OEN(cs),
				MPMC_STATIC_WAIT_OEN_MAX, 0);
	if (ret)
		goto fail;

	ret = pl172_timing_prop(np, "mpmc,read-enable-delay", adev,
				MPMC_STATIC_WAIT_RD(cs),
				MPMC_STATIC_WAIT_RD_MAX, 1);
	if (ret)
		goto fail;

	ret = pl172_timing_prop(np, "mpmc,page-mode-read-delay", adev,
				MPMC_STATIC_WAIT_PAGE(cs),
				MPMC_STATIC_WAIT_PAGE_MAX, 1);
	if (ret)
		goto fail;

	ret = pl172_timing_prop(np, "mpmc,write-delay", adev,
				MPMC_STATIC_WAIT_WR(cs),
				MPMC_STATIC_WAIT_WR_MAX, 2);
	if (ret)
		goto fail;

	ret = pl172_timing_prop(np, "mpmc,turn-round-delay", adev,
				 ...
				 struct device_node *np)
{
	...
	if (!of_property_read_u32(np, "mpmc,cs", &cs)) {
		if (cs >= PL172_MAX_CS) {
			dev_err(&adev->dev, "Invalid cs %u\n", cs);
			return -EINVAL;
		}

		return pl172_configure_static(cs, adev, np);
	}
...
static const char *pl172_revision(struct amba_device *adev)
{
	if (amba_part(adev) == 0x172) {
		switch (amba_rev(adev)) {
			case 0: return "r1";
			case 1: return "r2";
			case 2: return "r2p3";
			case 3: return "r2p4";
...
static int pl172_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	if (IS_ERR(pl172->clk)) {
		dev_err(dev, "Can't find mpmcclk clock.\n");
	...
	if (!pl172->rate) {
		dev_err(dev, "Can't get clock rate.\n");
		ret = -EINVAL;
		goto err_clk_enable;
	}
	...
	if (ret) {
		dev_err(dev, "Can't request AMBA IO region.\n");
		goto err_clk_enable;
	}
